...
public class TelemetryEventProcessor extends Thread {
    ...
    public void run() {
        try {
            ...
            properties.put("group.id", "grid-balancing-telemetry-group");

            try (Consumer<String, String> consumer = new KafkaConsumer<>(properties)) {
                consumer.subscribe(Arrays.asList("Telemetry-Batteries", "Telemetry-Solar", "Telemetry-Chargers"));

                System.out.println("TelemetryEventProcessor: Subscribed to Telemetry-Batteries, Telemetry-Solar, Telemetry-Chargers for grid balancing");
            ...
            System.out.println("Exception in TelemetryEventProcessor (GridBalancing): " + e);
            e.printStackTrace();
        }
    }
    ...
    private void calculateRecommendations() {
        try {
            ...
            List<GridCellDTO> gridCells = client.query("SELECT gridCellId, utilityOperatorId, maxCapacity, geographicBoundaries FROM GridCell")
                .execute()
                .map(rows -> {
                    List<GridCellDTO> cells = new ArrayList<>();
                    rows.forEach(row -> {
                        GridCellDTO cell = new GridCellDTO();
                        cell.gridCellId = row.getString("gridCellId");
                        cell.utilityOperatorId = row.getLong("utilityOperatorId");
                        cell.maxCapacity = row.getDouble("maxCapacity");
                        cell.geographicBoundaries = row.getString("geographicBoundaries");
                        cells.add(cell);
                    });
                    return cells;
                })
            ...
            System.out.printf("GridBalancing: Calculating recommendations with %d telemetry events and %d grid cells\n",
                telemetryList.size(), gridCells.size());

            recommendationService.calculateRecommendationsFromEvents(telemetryList, gridCells)
                .subscribe().with(
                    recommendations -> System.out.printf("GridBalancing: Created %d recommendations\n", recommendations.size()),
                    failure -> System.err.println("GridBalancing: Failed to calculate recommendations: " + failure.getMessage())
                );
        } catch (Exception e) {
            System.err.println("GridBalancing: Error calculating recommendations: " + e.getMessage());
            e.printStackTrace();
        }
    }
}
